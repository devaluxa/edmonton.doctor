...
import { baseNavigation, clinicServicePages } from "../../lib/sitePages";
      ...
      <nav className="flex h-20 items-center justify-between gap-4 px-4 sm:px-5 lg:h-[92px] lg:px-6 xl:px-10">
        ...
        <ul className="hidden min-w-0 items-center gap-4 xl:gap-6 lg:flex">
          ...
          >
            ...
            <div
              className={`absolute left-1/2 top-full w-[min(760px,calc(100vw-3rem))] -translate-x-1/2 pt-4 ${
                isServicesOpen ? "block" : "hidden"
              }`}
            >
              <div className="grid gap-4 rounded-lg border border-[var(--brand-color-border)] bg-white p-4 shadow-2xl shadow-slate-950/12 lg:grid-cols-[0.85fr_1.15fr]">
                <a
                  className="block rounded-md bg-[var(--brand-color-paper)] p-4 transition hover:bg-teal-50"
                  href="/services/"
                >
                  <span className="text-xs font-black uppercase tracking-[0.18em] text-[var(--brand-color-primary-strong)]">
                    All Services
                  </span>
                  ...
                  <span className="mt-2 block text-sm leading-6 text-slate-600">
                    Browse family medicine, walk-in care, registration, and
                    preventative health pages.
                  </span>
                </a>
                ...
                <div>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {clinicServicePages.map((item) => (
                      <a
                        className="rounded-md px-3 py-2 text-sm font-bold text-slate-700 transition hover:bg-teal-50 hover:text-[var(--brand-color-primary-strong)]"
                        href={item.path}
                        key={item.slug}
                      >
                        {item.eyebrow}
